...
from src.experiments.experiment_03.E03_config import e03_config
...
if __name__ == "__main__":
    ...
    # Set the e03_config['dataset_name"] parameter to complete dataset
    e03_config['dataset_name'] = 'ieeegrss_2023dfc_track2'

    # Initialize dataset dictionary
    data_dict = dataset_init(e03_config, seed)
    ...
    model = model_init(e03_config)

    # Define optimizer
    optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=e03_config['learning_rate'])

    # Train the model on full dataset
    train(model, train_loader, test_loader, optimizer, e03_config['loss_fn'], device, e03_config['epochs'], e03_config['save_name'], e03_config['save_path'])
    ...
    for dataset_name, dataset_folder_name in tqdm(DATASETS.items(), desc="Training and Evaluating Models", unit="sub-dataset"):
        # Set the e03_config parameter "dataset-_name"
        e03_config['dataset_name'] = dataset_folder_name

        # Initialize subdataset and their loaders
        sub_data_dict = dataset_init(e03_config, seed)
        ...
        with suppress_output():
            cross_evaluations[dataset_name] = model_evaluator(model, sub_train_loader, sub_test_loader, device, e03_config)
    ...
    for dataset_name, dataset_folder_name in tqdm(DATASETS.items(), desc="Training and Evaluating Models", unit="sub-dataset"):
        ...
        criterion = e03_config['loss_fn']
        optimizer = optim.Adam(filter(lambda p: p.requires_grad, model.parameters()), lr=e03_config['learning_rate'])

        # Train the model on full dataset
        with suppress_output():
            train(model, sub_train_loader, sub_test_loader, optimizer, e03_config['loss_fn'], device, e03_config['epochs'], e03_config['save_name'], e03_config['save_path'])
        
        # Evaluate the Model on Train and Test Sets and print the corresponding metric
        with suppress_output():
            individual_evaluations[dataset_name] = model_evaluator(model, sub_train_loader, sub_test_loader, device, e03_config)
